...
async function runReport(type, outputEl, actionsEl) {
  outputEl  = outputEl  || $('report-output');
  actionsEl = actionsEl || $('report-actions');
  outputEl.innerHTML = '<div class="text-muted"><span class="spinner"></span> Loading…</div>';
  show(actionsEl);
 
  switch (type) {
    case 'bidder':  await reportBidderSummary(outputEl);      break;
    case 'unsold':  await reportUnsoldQuilts(outputEl);       break;
    case 'unpaid':  await reportUnpaidBidders(outputEl);      break;
    case 'methods': await reportPaymentMethods(outputEl);     break;
    case 'nocard':  await reportBiddersWithoutCard(outputEl); break;
  }
}
 
// ── Report 1: Complete Bidder Summary ────────────────────────
 
async function reportBidderSummary(outputEl) {
  ...
  if (!bids || bids.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">No finalized bids to report.</div>';
    return;
  }
...
async function reportUnsoldQuilts(outputEl) {
  ...
  if (unsold.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">All quilts have been sold.</div>';
    return;
  }
...
async function reportUnpaidBidders(outputEl) {
  ...
  if (!bids || bids.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">No finalized bids.</div>';
    return;
  }
  ...
  if (rows.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">All bidders have paid in full.</div>';
    return;
  }
...
async function reportPaymentMethods(outputEl) {
  const { data: lines } = await sb.from('payment_lines').select('amount, method');
 
  if (!lines || lines.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">No payments recorded yet.</div>';
    return;
  }
...
async function reportBiddersWithoutCard(outputEl) {
  ...
  if (error) {
    outputEl.innerHTML = '<div class="alert alert-needs">Could not load bidders.</div>';
    return;
  }
 
  if (!bidders || bidders.length === 0) {
    outputEl.innerHTML = '<div class="empty-state">All bidders have been assigned a card.</div>';
    return;
  }
